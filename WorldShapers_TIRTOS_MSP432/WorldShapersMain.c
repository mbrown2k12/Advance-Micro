...
level_t Levels[NUMLEVELS] = {
  {3,    0, 2048,  FIX/8, 1, LCD_GREEN,      60,0}, // 0: rolling hills
  {4,    0, 2048,  FIX/4, 1, LCD_BLUE,        8,1}, // 1: over lake
  {5,  512, 4096,  FIX/2, 2, LCD_ORANGE,     40,0}, // 2: low lands (monsters start to shoot back)
  {6,  680, 4096,  FIX,  10, LCD_MAGENTA,    60,1}, // 3: mckenzie's city
  {7, 1024, 4096,  FIX,   2, LCD_LIGHTGREEN, 80,0}, // 4: mountains (Has big monsters)
  {8, 2048, 4096,  FIX,  10, LCD_GREY,       30,1}  // 5: city
};
...
void Sound_Shoot(void){
  buzzerDuration=.01*BUZZ100MS; //  Edited
}
void Sound_Killed(void){
  buzzerDuration=5*BUZZ100MS; // 0.5sec
}
void Sound_Explosion(void){
  buzzerDuration=20*BUZZ100MS; // 2sec Edited
}
...
void GameThread(void){ // runs at about 30 Hz
  uint16_t x,y; uint8_t button, button1;
  ...
  while(Things[SHIP].life){
    ...
    if(IntermissionFlag){
      BSP_Joystick_Input(&x,&y,&button, &button1);
...
uint8_t current, current1;
uint8_t static last=1;
  current = BSP_Button1_Input();
  current1 = BSP_Button2_Input();
  ...
  if((last)&&(current1 == 0)){     // Button2 was pressed
   // BSP_RGB_D_Set(1,0,0); // red
    i = FindFreeRocket();
    if(i){
      Sound_Shoot();
 ...
 */
int main(void){
  uint16_t x, y; uint8_t select; uint8_t select1; // joystick used to seed random number
  ...
  BSP_Joystick_Input(&x, &y,  &select, &select1);
  Random_Init(x+y+select+select1);
